...
class PrettyPrinter:
    ...
    def visit_function_definition(self, func_def):
        ...
        self.spaces += 4
        string_to_return += make_string_of_expr(func_def.function.body,
                                                self.spaces, self)
        self.spaces -= 4
    ...
    def visit_conditional(self, conditional):
        ...
        self.spaces += 4
        string_to_return += make_string_of_expr(conditional.if_true,
                                                self.spaces, self)
        string_to_return += "{}}} else {{\n".format(
                    " " * (self.spaces - 4))
        string_to_return += make_string_of_expr(conditional.if_false,
                                                self.spaces, self)
        self.spaces -= 4
        string_to_return += "{}}}".format(" " * self.spaces)
        return string_to_return
    ...
    def visit_function_call(self, func_call):
        string_of_args = ", ".join(arg.accept(self)
...
def make_string_of_expr(list_of_expr, spaces, visitor):
    return "".join("{}{};\n".format(" " * spaces, expr.accept(visitor))
                   for expr in list_of_expr or [])
